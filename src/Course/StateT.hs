...
instance Monad k => Monad (StateT s k) where
  ...
  f =<< sa = StateT $ \s -> runStateT sa s >>= \(a, s') -> runStateT (f a) s'
...
runState' st s | ExactlyOne as <- runStateT st s = as
...
distinct' as = eval' (filtering notMember' as) S.empty
  where
    notMember' :: Ord a => a -> State' (S.Set a) Bool
    notMember' a = getT >>= lift2 (<$) (S.notMember a) (putT . S.insert a)
...
distinctF as = evalT (filtering pred as) S.empty
  where
    pred :: a -> StateT (S.Set a) Optional Bool
    pred a = getT >>= \seen ->
      if a > 100
        then StateT (const Empty)
        else S.notMember a seen <$ putT (S.insert a seen)
...
instance Monad k => Applicative (OptionalT k) where
  ...
  (<*>) :: forall a b. OptionalT k (a -> b) -> OptionalT k a -> OptionalT k b
  OptionalT kf <*> OptionalT ka = OptionalT $ kf >>= onFull (<$$> ka)
...
instance Monad k => Monad (OptionalT k) where
  (=<<) :: forall a b. (a -> OptionalT k b) -> OptionalT k a -> OptionalT k b
...
instance Monad (Logger l) where
  ...
  f =<< Logger l1 a = Logger (l1 ++ l2) b where Logger l2 b = f a 
