...
-- >>> sequence ((*10) :. (+2) :. Nil) 6
-- [60,8]
sequence :: forall k a. Applicative k => List (k a) -> k (List a)
...
replicateA :: forall k a. Applicative k => Int -> k a -> k (List a)
...
filtering p as = flatten <$> sequence (f <$> as) 
  where
    f :: a -> k (List a)
    f a = lift1 (\b -> if b then (a :. Nil) else Nil) (p a)
